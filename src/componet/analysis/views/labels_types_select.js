...
class LabelsTypesSelect extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handelLableTypeClick = (event) => {
    ...
    this.props.handelLabelsTypessBack(select_labels_types);
    ...
    return (
        ...
        <Card.Body>
          ...
          <Form.Control as="select" multiple value={this.props.item.select_labels_types} onChange={this.handelLableTypeClick}>
            {typeof (this.props.labels_types) == 'undefined' ? '' : this.props.labels_types.map((row, index) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(LabelsTypesSelect);
