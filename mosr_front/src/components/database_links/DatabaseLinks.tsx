...
import { axios_jsonput, axios_response, axios_jsonpost } from '../../public_func/_axios'
						...
						axios_response(
							{
								...
								setLastModified(database_link_data.d_add_datetime)
							...
							'array_datas': [
								{
									...
									'd_password': db_password,
									'd_url': d_url,
									'd_add_datetime': dateFormatterFull(new Date()),
									'd_add_username': _globalState.user_name,
									'd_alias': db_alias,
									'd_memo': db_memo,
								...
								axios_response(
									{
										...
										setConnecting(false)
										//console.log(check_data)
										check_data.status ===  true ? setOkButtonDisable(false) : Modal.error({ title: '测试数据库连接',  content: check_data.message, })
										//更新登陆状态
										//_globalDispatch({ 'type': 'login_success', 'payload': { 'permission': permission_data, 'user_name': login_data.u_user_name, 'nickname': login_data.u_nickname, 'user_uuid': login_data.u_uuid, 'user_last_login_datetime': login_data.u_last_login_datetime } });
									}
								)
